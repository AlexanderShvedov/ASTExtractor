...
public class Main {
    ...
    private static final String filePath = "out_99.java";
    ...
    private static void visitNode(StringBuffer result, String indent, ASTNode node) {
        ...
        if (ASTExtractorProperties.OMIT.contains(nodeType)) {
            ...
            if (nodeType.equals("PrefixExpression")) {
                result.append(indent + "<" + nodeType + " text=\"" + ((PrefixExpression)node).getOperator().toString() + "\"" + ">\n");
            } else if (nodeType.equals("InfixExpression")) {
                result.append(indent + "<" + nodeType + " text=\"" + ((InfixExpression)node).getOperator().toString().replace("&", "&amp;") + "\"" + ">\n");
            } else {
                result.append(indent + "<" + nodeType + ">\n");
            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        String ast = stringBuilder.toString();
        String xml = parse(ast);
